...
module.exports = grammar({
  ...
  rules: {
    ...
    identifier: ($) =>
      token(
        seq(choice(/\p{ID_Start}/, "_"), repeat(choice(/\p{ID_Continue}/, "-")))
      ),
    ...
    string_lit: ($) =>
      prec(
        PREC.string_lit,
        seq($.quoted_template_start, $.template_literal, $.quoted_template_end)
      ),
    ...
    object: ($) => seq($.object_start, optional($._object_elems), $.object_end),
    ...
    template_if_branch: ($) => seq($.template_if_intro, $._template),
    ...
    template_else_branch: ($) => seq($.template_else_intro, $._template),
